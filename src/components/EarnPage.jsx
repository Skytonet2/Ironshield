...
export default function EarnPage({ openWallet }) {
  ...
  // Use memoryStore (from store.js) as source of truth — no localStorage
  const [contests, setContests]       = useState(memoryStore.contests);
  const [submitted, setSubmitted]     = useState([]);
  const [leaderboard, setLeaderboard] = useState([]);

  useEffect(() => {
    // Derive leaderboard from memoryStore.scores
    ...
    // Keep submissions in sessionStorage (temporary until governance contract is live)
    try {
  ...
  const doSubmit = () => {
    ...
    sessionStorage.setItem("ironshield_submissions", JSON.stringify(updated));
    setSubmitting(null);
    setSubLink("");
    setSubNote("");
    setSubImg(null);
    ...
    <Section style={{ paddingTop: 100 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 12, padding: "12px 20px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: t.amber }}>
              {connected && userRankData ? `${userRankData.badge} #${userRankData.rank}` : "—"}
            </div>
          </div>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 12, padding: "12px 20px", textAlign: "center" }}>
            <div style={{ fontSize: 11, color: t.textMuted }}>Points</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: t.green, fontFamily: "'JetBrains Mono', monospace" }}>
              {connected && userRankData ? userRankData.pts.toLocaleString() : "—"}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 28, borderBottom: `1px solid ${t.border}`, paddingBottom: 12 }}>
        {[
          ...
          { key: "submissions",  label: `My Submissions (${submitted.length})` },
        ...
        <div>
          <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 10, padding: "10px 14px" }}>
              ...
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search missions..."
                style={{ background: "none", border: "none", outline: "none", color: t.text, fontSize: 13, flex: 1 }} />
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {filtered.length === 0 && (
              <div style={{ textAlign: "center", padding: "40px 0", color: t.textMuted }}>No missions found.</div>
            )}
            {filtered.map(task => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
                  <div style={{ background: t.bgSurface, borderRadius: 10, width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
                    {task.emoji || "🎯"}
                  </div>
                  <div>
        ...
        <div>
          {!connected ? (
            <div style={{ textAlign: "center", padding: "60px 0" }}>
              ...
              <Btn primary onClick={openWallet}>Connect Wallet</Btn>
            </div>
          ) : submitted.length === 0 ? (
                ...
                <div key={i} style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 14, padding: 24 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    ...
                    <a href={sub.link} target="_blank" rel="noopener noreferrer"
                      style={{ color: t.accent, fontSize: 13, textDecoration: "none", wordBreak: "break-all" }}>{sub.link}</a>
                  </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(6px)" }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 36, width: 500, maxWidth: "90vw", maxHeight: "85vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
              ...
              <button onClick={() => setSubmitting(null)} style={{ background: t.bgSurface, border: `1px solid ${t.border}`, borderRadius: 8, width: 32, height: 32, cursor: "pointer", color: t.textMuted, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <X size={14} />
              </button>
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: t.bgSurface, border: `1px solid ${t.border}`, borderRadius: 8, padding: "10px 14px" }}>
                ...
                <input value={subLink} onChange={e => setSubLink(e.target.value)} placeholder="https://twitter.com/..."
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <textarea value={subNote} onChange={e => setSubNote(e.target.value)} rows={3} placeholder="Describe your submission..."
                style={{ width: "100%", background: t.bgSurface, border: `1px solid ${t.border}`, borderRadius: 8, padding: 12, color: t.text, fontSize: 13, outline: "none", resize: "vertical", fontFamily: "inherit" }} />
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              {subImg ? (
                <div style={{ position: "relative", display: "inline-block" }}>
                  ...
                  <button onClick={() => setSubImg(null)} style={{ position: "absolute", top: 6, right: 6, background: t.red, border: "none", borderRadius: 6, width: 24, height: 24, cursor: "pointer", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <X size={12} />
                  </button>
                </div>
              ) : (
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <Btn primary onClick={doSubmit} disabled={!subLink.trim()} style={{ flex: 1, justifyContent: "center" }}>
